...
class PPOIdxDataset(Dataset):
    def __init__(self, tokenizer):
        ...
        self.f = open("/search/ai/kaitongyang/ppo_glm_debug/data/same_long_prompt_di_logits_0.6_0.4/prompt.txt")
        with open("/search/ai/kaitongyang/ppo_glm_debug/data/same_long_prompt_di_logits_0.6_0.4/dataset_tmp.id", 'rb') as fp:
...
for cur_big_epoch in range(10):
    for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
        ...
        if (epoch+1) % 50 == 0 and str(ppo_trainer.accelerator.device) == "cuda:0":
            reward_path = "/search/ai/jamsluo/GLM_RLHF/ppo_glm/RLHF_MODEL_04_06_glm_fb16"
