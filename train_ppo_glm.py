...
class GLMPPOTrainer(PPOTrainer):
    def generate(self, inputs, gen_len):
        ...
        response = self.accelerator.unwrap_model(self.model).generate(**inputs, eos_token_id=50007, max_length=512, min_length=-1, top_k=0, top_p=1.0, repetition_penalty=1.3, do_sample=True)
        return response
