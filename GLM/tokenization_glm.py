...
class GLMTokenizerMixin:
    ...
    def build_inputs_for_generation_from_tensor(self, input_ids, targets, max_gen_length=512, padding=False):
        mask_ids = self.mask_token_ids
        attention_mask = torch.ones_like(input_ids).to(input_ids.device)
        targets = [target + [self.pad_token_id] * (max_gen_length + 1 - len(target)) for target in targets.cpu().tolist()]
    ...
    def build_inputs_for_generation_test(self, model_input: BatchEncoding, max_gen_length=512, targets=None, padding=False):
        mask_ids = self.mask_token_ids
