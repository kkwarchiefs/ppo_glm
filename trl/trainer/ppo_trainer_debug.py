...
class PPOTrainer(BaseTrainer):
    ...
    def batched_forward_pass(
        ...
        for i in range(int(bs / fbs)):
            ...
            input_kwargs["is_ref"] = is_ref
            query_batch = queries[i * fbs : (i + 1) * fbs]
            response_batch = responses[i * fbs : (i + 1) * fbs]
            logits, _, values, query_temp_len = model(**input_kwargs)

            if is_ref:
                logits.to(self.accelerator.device)
                values.to(self.accelerator.device)
            ...
            input_ids = torch.cat([input_kwargs["input_ids"], input_kwargs["decoder_input_ids"]], dim=1)
    ...
    def loss(
        ...
        print("vf_losses1, vf_losses2", vf_losses1, vf_losses2)
        ...
        print("pg_loss, vf_loss", pg_loss, vf_loss)
    ...
    def log_stats(
        ...
        if self.accelerator.is_main_process:
            ...
            print("="*20)
            print("reward_mean", logs["env/reward_mean"])
