...
config = PPOConfig(
    model_name="/search/ai/kaitongyang/RLHF_DEBUG/PPO_trl/glm-10b-chinese/80000",
    learning_rate=1e-5,
    batch_size=8,
    ppo_epochs=3,
    log_with="wandb",
...
for cur_big_epoch in range(10):
    for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
        ...
        '''
        rm_texts = [q.replace("[gMASK]", "").replace("<|startofpiece|>", "").replace("[回答]", "").replace("[CLS]", "").replace("\n", "") + r.replace("<|startofpiece|>", "").replace("<|endofpiece|>", "").replace("<|endoftext|>", "").replace("<n>","").replace(" ","") for q,r in zip(batch['query'], batch['response'])]
        rm_texts_temp = []
        for i_temp in rm_texts:
            rm_texts_temp.append(i_temp[:500])
        ...
        if epoch%20 == 0 and str(ppo_trainer.accelerator.device) == "cuda:0":
            reward_path = "/search/ai/kaitongyang/RLHF_DEBUG/PPO_trl/RLHF_MODEL_big_glm_fb16"
